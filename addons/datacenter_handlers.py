import config
from strings import datacenters, ui
...
def africa():
    ...
    capacity_ru, load_ru = array_ru
    africa_text_ru = f"{datacenters.africa_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    africa_text_en = f"{datacenters.africa_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def australia():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    australia_text_ru = f"{datacenters.australia_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    australia_text_en = f"{datacenters.australia_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def eu_north():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    eu_north_text_ru = f"{datacenters.north_eu_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    eu_north_text_en = f"{datacenters.north_eu_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def eu_west():
    ...
    eu_west_text_ru = f"{datacenters.west_eu_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    eu_west_text_en = f"{datacenters.west_eu_en.format(load, capacity, load_secondary, capacity_secondary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def eu_east():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru, capacity_secondary_ru, load_secondary_ru = array_ru
    eu_east_text_ru = f"{datacenters.east_eu_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    eu_east_text_en = f"{datacenters.east_eu_en.format(load, capacity, load_secondary, capacity_secondary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def usa_north():
    try:
        ...
        capacity_secondary, load_secondary = (
            sterling_dc["capacity"],
            sterling_dc["load"],
        )
        capacity_tertiary, load_tertiary = (
            moseslake_dc["capacity"],
            moseslake_dc["load"],
        )
    except:
        capacity = (
            ...
            capacity_secondary
        ) = load_secondary = capacity_tertiary = load_tertiary = "unknown"
    array = [
        capacity,
        load,
        capacity_secondary,
    ...
    usa_north_text_ru = f"{datacenters.north_us_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru, load_tertiary_ru, capacity_tertiary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    usa_north_text_en = f"{datacenters.north_us_en.format(load, capacity, load_secondary, capacity_secondary, load_tertiary, capacity_tertiary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def usa_south():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru, capacity_secondary_ru, load_secondary_ru = array_ru
    usa_south_text_ru = f"{datacenters.south_us_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    usa_south_text_en = f"{datacenters.south_us_en.format(load, capacity, load_secondary, capacity_secondary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def south_america():
    try:
        ...
        capacity_quaternary, load_quaternary = (
            argentina_dc["capacity"],
            argentina_dc["load"],
        )
    except:
        capacity = (
            ...
            capacity_tertiary
        ) = load_tertiary = capacity_quaternary = load_quaternary = "unknown"
    array = [
        capacity,
        load,
        capacity_secondary,
    ...
    (
        ...
        capacity_tertiary_ru,
        load_tertiary_ru,
        capacity_quaternary_ru,
        load_quaternary_ru,
    ) = array_ru
    south_america_text_ru = f"{datacenters.south_america_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru, load_tertiary_ru, capacity_tertiary_ru, load_quaternary_ru, capacity_quaternary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    south_america_text_en = f"{datacenters.south_america_en.format(load, capacity, load_secondary, capacity_secondary, load_tertiary, capacity_tertiary, load_quaternary, capacity_quaternary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def india():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru, capacity_secondary_ru, load_secondary_ru = array_ru
    india_text_ru = f"{datacenters.india_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    india_text_en = f"{datacenters.india_en.format(load, capacity, load_secondary, capacity_secondary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def japan():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    japan_text_ru = f"{datacenters.japan_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    japan_text_en = f"{datacenters.japan_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def china():
    ...
    china_text_ru = f"{datacenters.china_ru.format(load_ru, capacity_ru, load_secondary_ru, capacity_secondary_ru, load_tertiary_ru, capacity_tertiary_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    china_text_en = f"{datacenters.china_en.format(load, capacity, load_secondary, capacity_secondary, load_tertiary, capacity_tertiary)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def emirates():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    emirates_text_ru = f"{datacenters.emirates_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    emirates_text_en = f"{datacenters.emirates_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def singapore():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    singapore_text_ru = f"{datacenters.singapore_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    singapore_text_en = f"{datacenters.singapore_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def hong_kong():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    hong_kong_text_ru = f"{datacenters.hong_kong_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    hong_kong_text_en = f"{datacenters.hong_kong_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
...
def south_korea():
    ...
    for data in array:
        ...
        array_ru.append(data_ru)
    capacity_ru, load_ru = array_ru
    south_korea_text_ru = f"{datacenters.south_korea_ru.format(load_ru, capacity_ru)}\n\n{ui.last_upd_ru.format(tsRCache)}"
    south_korea_text_en = f"{datacenters.south_korea_en.format(load, capacity)}\n\n{ui.last_upd_en.format(tsCache)}"
